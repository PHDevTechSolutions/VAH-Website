...
function ApplyFormContent() {
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] font-sans selection:bg-[#dcb485]/10">
      <nav className="max-w-7xl mx-auto p-6 md:p-10">
        <Link href="/careers" className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 hover:text-[#dcb485] transition-all group">
          <div className="p-2 rounded-full bg-white shadow-sm group-hover:bg-amber-50 transition-all">
            <ArrowLeft size={14} />
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-5 space-y-8">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <span className="text-[#dcb485] text-[11px] font-black uppercase tracking-[0.4em] mb-4 block italic">
              ...
              <div className="flex flex-wrap gap-4 mt-6">
                <div className="flex items-center gap-2 text-[10px] font-bold text-gray-500 uppercase tracking-widest bg-white px-4 py-2 rounded-full border border-gray-100">
                  <MapPin size={12} className="text-[#dcb485]" /> {jobData?.location || "Remote / Office"}
                </div>
                <div className="flex items-center gap-2 text-[10px] font-bold text-gray-500 uppercase tracking-widest bg-white px-4 py-2 rounded-full border border-gray-100">
                  <Clock size={12} className="text-[#dcb485]" /> {jobData?.jobType || "Full-time"}
                </div>
              </div>
            </motion.div>
            ...
            >
              <div>
                <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-[#dcb485] mb-4">Job Overview</h3>
              ...
              {jobData?.qualifications && jobData.qualifications.length > 0 && (
                <div>
                  <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-[#dcb485] mb-4">Key Qualifications</h3>
                  <ul className="space-y-3">
                    {jobData.qualifications.map((q: string, i: number) => (
                      ...
                      >
                        <CheckCircle2 size={16} className="text-[#dcb485] mt-0.5 shrink-0" />
                        <span>{q}</span>
                      </motion.li>
                    ))}
                  </ul>
            ...
            <form onSubmit={handleSubmit} className="space-y-7">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-7">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input required name="fullName" type="text" placeholder="Juan Dela Cruz" className="w-full pl-14 pr-6 py-5 bg-gray-50 border-2 border-transparent rounded-[1.5rem] focus:bg-white focus:border-[#dcb485] transition-all outline-none text-sm font-bold" />
                  </div>
                </div>
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input required name="email" type="email" placeholder="juan@gmail.com" className="w-full pl-14 pr-6 py-5 bg-gray-50 border-2 border-transparent rounded-[1.5rem] focus:bg-white focus:border-[#dcb485] transition-all outline-none text-sm font-bold" />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input required name="phone" type="tel" placeholder="0912 345 6789" className="w-full pl-14 pr-6 py-5 bg-gray-50 border-2 border-transparent rounded-[1.5rem] focus:bg-white focus:border-[#dcb485] transition-all outline-none text-sm font-bold" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <label htmlFor="resume" className="flex flex-col items-center justify-center p-12 border-2 border-dashed border-gray-200 rounded-[2.5rem] bg-gray-50 hover:bg-amber-50 hover:border-[#dcb485] transition-all cursor-pointer group text-center">
                    <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center shadow-sm mb-4 group-hover:scale-110 transition-transform">
                      {isSubmitting ? <Loader2 className="animate-spin text-[#dcb485]" size={32} /> : (fileName ? <CheckCircle2 className="text-green-500" size={32} /> : <UploadCloud className="text-[#dcb485]" size={32} />)}
                    </div>
              ...
              <div className="pt-4">
                <button type="submit" disabled={isSubmitting} className="w-full bg-gray-900 text-white py-6 rounded-[1.5rem] font-black uppercase text-[12px] tracking-[0.3em] shadow-xl hover:bg-[#dcb485] transition-all flex items-center justify-center gap-3 active:scale-[0.98] disabled:opacity-50">
